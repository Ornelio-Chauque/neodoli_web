...
express()
  ...
  .post("/dashboard/:id/response/",bodyParse.urlencoded({extended:false}), (req, res)=>{
    ...
    db.db.one('INSERT INTO responses (pharmacy, address, "totalAmount", "prescriptionId") VALUES($1, $2, $3, $4) RETURNING *', [req.body.name, req.body.address, req.body.amount, req.params.id])
    .then(response=>{
  ...
  .get("/dashboard/:id/response/:responseId/medicines", (req, res)=>{
    
  })

  .post("/dashboard/:id/response/:responseId/medicine", bodyParse.urlencoded({extended:true}),(req, res)=>{
    db.db.none('INSERT INTO medicines(name, available, "madeIn", "prescriptionId" VALUES($1, $2, $3, $4)', [req.body.name,req.body.available, req.body.madeIn, req.params.prescriptionId ])
    .then(data=>{
