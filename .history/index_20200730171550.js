...
passport.use(new LocalStrategy({passReqToCallback : true },(username, password, cb)=>{
  ...
  db.getUserWithUsername(username, password, (error, user)=>{
    ...
    if(!user){
      ...
      return cb(null, false,req.flash("message","no user found in auth process"));
    }
...
express()
  ...
  .get('/login', (req, res)=>{
    ...
    res.render("pages/login", {title: 'Iniciar sessão', message:req.flash("message")});
  })
  .post('/login',
